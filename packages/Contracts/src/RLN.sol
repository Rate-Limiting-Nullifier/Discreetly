...
contract RLN {
    ...
    modifier withdrawEnabled() {
        require(withdrawPaused == 0);
        _;
    }

    modifier slashingEnabled() {
        require(slashingPaused == 0);
        _;
    }

    modifier registrationEnabled() {
        require(registrationPaused == 0);
        _;
    }
    ...
    function register(
        ...
        require(
            amount >= MINIMAL_DEPOSIT,
            "RLN, register: amount is lower than minimal deposit"
        );
        require(
            amount % MINIMAL_DEPOSIT == 0,
            "RLN, register: amount should be a multiple of minimal deposit"
        );
        require(
            members[identityCommitment].userAddress == address(0),
            "RLN, register: idCommitment already registered"
        );

        token.safeTransferFrom(msg.sender, address(this), amount);
        uint256 messageLimit = amount / MINIMAL_DEPOSIT;

        members[identityCommitment] = User(
            msg.sender,
            messageLimit,
    ...
    ) external withdrawEnabled {
        User memory member = members[identityCommitment];
        require(
            member.userAddress != address(0),
            "RLN, withdraw: member doesn't exist"
        );
        require(
            withdrawals[identityCommitment].blockNumber == 0,
            "RLN, release: such withdrawal exists"
        );
        require(
            _verifyProof(identityCommitment, member.userAddress, proof),
            "RLN, withdraw: invalid proof"
        );

        uint256 withdrawAmount = member.messageLimit * MINIMAL_DEPOSIT;
        withdrawals[identityCommitment] = Withdrawal(
            block.number,
            withdrawAmount,
            member.userAddress
        );
    ...
    function release(uint256 identityCommitment) external withdrawEnabled {
        Withdrawal memory withdrawal = withdrawals[identityCommitment];
        require(
            withdrawal.blockNumber != 0,
            "RLN, release: no such withdrawals"
        );
        require(
            block.number - withdrawal.blockNumber > FREEZE_PERIOD,
            "RLN, release: cannot release yet"
        );
    ...
    function slash(
        ...
        address receiver,
        uint256[8] calldata proof
    ) external slashingEnabled {
        require(receiver != address(0), "RLN, slash: empty receiver address");

        User memory member = members[identityCommitment];
        require(
            member.userAddress != address(0),
            "RLN, slash: member doesn't exist"
        );
        require(
            member.userAddress != receiver,
            "RLN, slash: self-slashing is prohibited"
        );

        require(
            _verifyProof(identityCommitment, receiver, proof),
            "RLN, slash: invalid proof"
        );
    ...
    function owner_register(uint256 identityCommitment) external ownerOnly {
        ...
        emit MemberRegistered(identityCommitment, 0, identityCommitmentIndex);
